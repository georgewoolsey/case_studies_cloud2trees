# Case Study 1: QUIC-fire Inputs for Prescribed Burn Planning

use raw point clouds to generate spatial fuel data (bulk density and canopy base height) for use as input into the coupled fire-atmosphere [QUIC-fire model](https://doi.org/10.1016/j.envsoft.2019.104616). Prescribed fire modelling enables managers to optimize prescribed burn plans for effective implementation and safety.
...
f <- "f:/lidar_phys_fire_mods/data/mogollon_rim_az_lidar/"
# is there data?
...
cloud2trees_ans$treetops_sf %>%
  ...
  ggplot2::geom_density(fill = "plum4", color = "plum4", alpha = 0.2) +
...
Note, we use the argument `dbh_model_regional = "power"` within the `trees_dbh()` function to use a nonlinear power function for regional dbh-height allometry based on FIA tree measurements with the form: 

$$
\text{DBH (cm)} = \theta_1 \cdot \text{Height (m)} + \text{Height (m)}^{\theta_2}
$$
